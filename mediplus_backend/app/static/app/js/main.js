(() => {
  ...
  */
  ...
  const $baseForm = select("#base-form")
  const baseFormData = new FormData($baseForm)

  const ajaxPost = (action, formData) => fetch(action, {
    method: 'POST',
    body: formData,
    headers: {'X-Requested-With': 'XMLHttpRequest'}
  })

  const ajaxDelete = (action) => fetch(action, {
    method: "DELETE",
    body: baseFormData,
    headers: {'X-Requested-With': 'XMLHttpRequest'}
  })
  ...
  $productElements.forEach(productElement => {
    ...
    if ($addWatchForm) {
      $addWatchForm.addEventListener("submit", async (e) => {
        ...
        let res = await ajaxPost($addWatchForm.getAttribute("action"), formData)
        try {
          if (res.ok) {
    ...
    onsubmit($addCartItemForm, async (e) => {
      e.preventDefault()
      let formData = new FormData($addCartItemForm)
      let res = await ajaxPost($addCartItemForm.getAttribute("action"), formData)
      try {
        if (res.ok) {
